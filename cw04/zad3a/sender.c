...
int sig_cnt = 0;
int catcher_sig_cnt=0; //for sigqueue
...
{
	if(info->si_value.sival_int!=0) catcher_sig_cnt=info->si_value.sival_int; //if sigqueue
	block_signals();
}
...
int main(int argc, char ** argv)
{
    ...
    if (strcmp(mode, "KILL") == 0)
    {
        ...
        for (int i = 0; i < no_signals; i++) kill(catcher_pid, SIGUSR1);
		sleep(1);
    ...
    else if (strcmp(mode, "SIGQUEUE") == 0)
    {
        ...
        printf("[MESSAGE FROM CATCHER] %d signals was received successfully.\n",catcher_sig_cnt);
    }
    else if (strcmp(mode, "SIGRT") == 0)
    {
        ...
        for (int i = 0; i < no_signals; i++) kill(catcher_pid, SIGRTMIN);
		sleep(1);
